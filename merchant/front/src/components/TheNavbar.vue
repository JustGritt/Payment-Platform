          ...
          <div class="flex flex-1 items-center justify-end">
            ...
            <Popover class="ml-4 z-50 flow-root text-sm lg:relative lg:ml-8">
              ...
              <transition enter-active-class="transition ease-out duration-200" enter-from-class="opacity-0"
                ...
                <PopoverPanel v-if="cartItemCount > 0"
                  class="absolute inset-x-0 top-16 mt-px bg-white pb-6 shadow-lg sm:px-2 lg:left-auto lg:right-0 lg:top-full lg:-mr-1.5 lg:w-80 lg:rounded-lg lg:ring-1 lg:ring-black lg:ring-opacity-5">
                      ...
                      <li v-for="product in cartItems" :key="product.id" class="flex items-center py-6">
                        <div class="mr-4">
                          <input type="number" v-model="product.quantity"
                            class="w-20 border-gray-200 rounded-md text-center"
                            @input="handleQuantityChange(product.id, product.quantity)" />
                        </div>
                        ...
                        <button @click="removeFromCart(product.id, product.quantity)"
                          class="ml-4 text-gray-700 hover:text-red-500 focus:outline-none">
                ...
                <PopoverPanel v-else
                  class="absolute inset-x-0 top-16 pt-4 mt-2 bg-white pb-6 shadow-lg sm:px-2 lg:left-auto lg:right-0 lg:top-full lg:-mr-1.5 lg:w-80 lg:rounded-lg lg:ring-1 lg:ring-black lg:ring-opacity-5">
                  <h2 class="sr-only">Shopping Cart</h2>
                  <div class="mx-auto max-w-2xl px-4">
                    <p class="text-center text-gray-700">No products in the cart</p>
                    <button @click="showEmptyCartPopover = false"
                      class="mt-4 w-full rounded-md border border-transparent bg-indigo-600 px-4 py-2 text-sm font-medium text-white shadow-sm hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 focus:ring-offset-gray-50">Close</button>
                  </div>
                </PopoverPanel>
              </transition>
            </Popover>
